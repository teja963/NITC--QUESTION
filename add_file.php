...
if(isset($_FILES['uploaded_file']) and isset ($_POST['Submit'])) {
    ...
    if($_FILES['uploaded_file']['error'] == 0) {
        ...
        $query = "
            INSERT INTO `file` (
                `name`, `Branch`, `Year`, `Subject`, `Q_paper`, `Roll_no`, `Id`,`mime`,`size`
            )
            VALUES (
                '{$name}','$Branch', $Year, '$Subject', '{$Q_paper}', '$Roll_no', 12345, '{$mime}', '{$size}'
            )";
...
echo '<p>Click <a href="qp.php">here</a> to go back</p>';
?>
